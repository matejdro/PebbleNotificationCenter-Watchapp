...
void closeApp(void)
{
	...
	reliable_app_message_write_uint8(iterator, 1, 3);
    reliable_app_message_write_uint8(iterator, 2, rejectNotifications ? 1 : 0);
    app_comm_set_sniff_interval(SNIFF_INTERVAL_REDUCED);
...
static void send_initial_packet() {
	...
	reliable_app_message_write_uint16(iterator, 2, PROTOCOL_VERSION);
	reliable_app_message_write_uint32(iterator, 3, getCapabilities(appmessage_max_size));
